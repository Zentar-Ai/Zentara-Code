	...
	useAppTranslation: () => ({
		t: (key: string, options?: { version: string }) => {
			...
			if (key === "chat:announcement.description") {
				return `Zentara Code ${options?.version} brings powerful new features and improvements based on your feedback.`
			}
...
describe("Announcement", () => {
	...
	it("renders the announcement with the version number from package.json", () => {
		...
		expect(
			screen.getByText(
				`Zentara Code ${expectedVersion} brings powerful new features and improvements based on your feedback.`,
			),
		).toBeInTheDocument()
	})
})
