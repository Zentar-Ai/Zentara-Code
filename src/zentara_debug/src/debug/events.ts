...
debugEvents.emit = function(event: string, ...args: any[]): boolean {
    if (event === DAP_STOPPED_EVENT) {
        ...
        outputChannel.appendLine(`[Debug Events ${new Date().toISOString()}] Emitting ${event} for session ${eventData.sessionId}. Stop reason: ${eventData.body?.reason}. Event body: ${JSON.stringify(eventData.body, null, 2)}`);
    }
...
export function updateLastKnownStopEvent(sessionId: string, body: any): void {
    outputChannel.appendLine(`[Debug Events ${new Date().toISOString()}] Updating last known stop event for session ${sessionId}. Previous session: ${lastKnownStopEventSessionId}. Stop reason: ${body?.reason}. Body: ${JSON.stringify(body, null, 2)}`);
...
export function clearActiveSessionDapOutputBuffer(sessionId: string): void {
    if (activeSessionDapOutputBuffers.has(sessionId)) {
        ...
        outputChannel.appendLine(`[DAP Output Buffer ${new Date().toISOString()}] Cleared active DAP output buffer for session ${sessionId}.`);
    }
}
...
export async function finalizeActiveSessionDapOutput(sessionId: string): Promise<void> {
    ...
    outputChannel.appendLine(`[DAP Output Buffer ${new Date().toISOString()}] Starting finalization for session ${sessionId}. Initial buffer length: ${lastLength}. Polling for quiescence...`);
    ...
    for (let i = 0; i < DAP_OUTPUT_QUIESCENCE_MAX_POLLS; i++) {
        ...
        if (currentLength === lastLength && currentLength > 0) {
            outputChannel.appendLine(`[DAP Output Buffer ${new Date().toISOString()}] DAP output quiesced with data for session ${sessionId} after ${i + 1} poll(s). Length: ${currentLength}.`);
            break;
        }
        ...
        if (i === DAP_OUTPUT_QUIESCENCE_MAX_POLLS - 1) {
            outputChannel.appendLine(`[DAP Output Buffer ${new Date().toISOString()}] DAP output polling reached max attempts for session ${sessionId}. Proceeding with finalization. Final length: ${currentLength}.`);
        } else if (currentLength === 0) { // No "else if lastLength === 0" needed, as lastLength would be 0 if currentLength is 0 and it's not the first iteration.
             outputChannel.appendLine(`[DAP Output Buffer ${new Date().toISOString()}] DAP output buffer still empty for session ${sessionId} after ${i + 1} poll(s). Continuing to poll.`);
        }
    }
    ...
    if (buffer && buffer.length > 0) {
        ...
        outputChannel.appendLine(`[DAP Output Buffer ${new Date().toISOString()}] Finalized DAP output for session ${sessionId}. Length: ${fullOutput.length}`);
    } else {
        outputChannel.appendLine(`[DAP Output Buffer ${new Date().toISOString()}] No active DAP output to finalize for session ${sessionId} after polling.`);
...
export function clearFinalizedSessionDapOutput(sessionId: string): void {
    if (terminatedSessionDapOutput.has(sessionId)) {
        ...
        outputChannel.appendLine(`[DAP Output Buffer ${new Date().toISOString()}] Cleared finalized DAP output for session ${sessionId}.`);
    }
}
