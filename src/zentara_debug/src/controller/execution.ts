...
function _buildNavigationResult(
    processedResult: Partial<NavigationResult>,
    operationName: string
): NavigationResult {
    const resultToReturn: NavigationResult = {
        success: processedResult.success ?? false,
        errorMessage: processedResult.success ? undefined : (processedResult.errorMessage || `Error processing event for ${operationName}`),
        frame: processedResult.frame,
        exceptionMessage: processedResult.exceptionMessage,
        stopReason: processedResult.stopReason,
        capturedDapOutput: processedResult.capturedDapOutput,
        capturedRawTerminalOutput: processedResult.capturedRawTerminalOutput,
    };
    outputChannel.appendLine(`[${operationName}] Returning ${resultToReturn.success ? 'success' : 'error'} result: ${stringifySafe(resultToReturn)}`);
...
async function pollUntilStateChange(
    ...
    outputChannel.appendLine(`[Polling Check] Starting polling for ${operationName} at ${new Date(startTime).toISOString()} with ${timeoutMs}ms timeout...`);
    ...
    while (Date.now() - startTime < timeoutMs) {
        ...
        try {
            ...
            if (traceResult.success && traceResult.frames && traceResult.frames.length > 0) {
                ...
                if (!initialTopFrame || currentTopFrame.id !== initialTopFrame.id || currentTopFrame.line !== initialTopFrame.line) { // Removed sourcePath check to match original
                    const elapsed = Date.now() - startTime;
                    outputChannel.appendLine(`[Polling Check] State changed confirmed after ${elapsed}ms for ${operationName}. Initial: ${initialTopFrame?.id}/${initialTopFrame?.line}, Current: ${currentTopFrame.id}/${currentTopFrame.line}`);
                    return;
                }
            } else if (!traceResult.success && traceResult.errorMessage?.includes('No active debug session')) {
                outputChannel.appendLine(`[Polling Check] Session terminated during stack trace attempt for ${operationName}.`);
                return;
            } else if (!traceResult.success) {
                outputChannel.appendLine(`[WARN][Polling Check] stackTrace failed during ${operationName}: ${traceResult.errorMessage}. Retrying...`);
            } else {
                outputChannel.appendLine(`[Polling Check] stackTrace returned no frames after ${operationName}. Assuming completion or termination.`);
                return;
            }
        } catch (pollError: any) {
             outputChannel.appendLine(`[WARN][Polling Check] Error during stackTrace poll for ${operationName}: ${pollError.message}. Retrying...`);
        }
        await new Promise(resolve => setTimeout(resolve, POLL_INTERVAL_MS));
    }
    const elapsed = Date.now() - startTime;
    outputChannel.appendLine(`[WARN][Polling Check] Timeout after ${elapsed}ms (limit: ${timeoutMs}ms) waiting for state change after ${operationName}.`);
}
...
async function sendDapRequest(
    ...
    args: any,
    operationName: string
): Promise<DebuggerResponse> { // Kept DebuggerResponse as return type
    ...
    try {
        await new Promise<void>((resolve, reject) => {
            ...
            thenable.then(
                ...
                (err: any) => { clearTimeout(timeoutId); reject(err); }
            );
        });
        ...
        if (error.message?.includes('session has been terminated')) {
             return { success: false, errorMessage: `Error ${operationName}: Debug session terminated unexpectedly during DAP request.` };
        }
    ...
    try {
        for (let attempts = 0; attempts < 5; attempts++) {
            ...
            const threadsResp = await new Promise<any>((resolveCustom, rejectCustom) => {
                const timeoutId = setTimeout(() => rejectCustom(new Error("DAP 'threads' request timed out")), STEP_TIMEOUT_MS); // Original used STEP_TIMEOUT_MS
                const thenable = session.customRequest('threads');
                thenable.then(
                    (response) => { clearTimeout(timeoutId); resolveCustom(response); },
                    (err: any) => { clearTimeout(timeoutId); rejectCustom(err); }
                );
            });
...
export async function _executeDapRequestAndProcessOutcome(
	...
	outputChannel.appendLine(`[${operationName}] Core helper. Session: ${activeSession.id}, LaunchInfo: ${stringifySafe(launchInfo)}`);

	try {
		let actionResult: { success: boolean; errorMessage?: string };

		if (launchInfo) {
			actionResult = { success: launchInfo.initialActionSuccess, errorMessage: launchInfo.initialActionErrorMessage };
            ...
            if (actionResult.success) {
                ...
                outputChannel.appendLine(`[${operationName}] Using pre-completed launch action: Failed (${actionResult.errorMessage}).`);
            }
		} else if (actionFn) {
		...
		if (!actionResult.success) {
			outputChannel.appendLine(`[${operationName}] DAP request failed: ${actionResult.errorMessage}. Allowing time for final output capture...`); // Matched original log for dapRequestResult.success false
			...
			return _buildNavigationResult({
				success: false,
				errorMessage: actionResult.errorMessage || `Session terminated during ${operationName} (DAP request failed).`, // Matched original
		...
		if (!stopResult || !stopResult.stopped) { // Covers termination or timeout from waitForDapStop
			outputChannel.appendLine(`[${operationName}] Session terminated during wait for stop event. Allowing time for final output capture...`);
			const sessionId = activeSession.id;
		...
		outputChannel.appendLine(`[${operationName}] Initial stop event received. Delaying ${EXCEPTION_CATCHUP_DELAY_MS}ms for potential concurrent exception processing...`);
		...
		if (
			...
			outputChannel.appendLine(`[${operationName}] Overriding initial stop event body with more detailed global exception event. Initial reason: ${bodyToProcess?.reason}, Global reason: ${lastKnownStopEventBody.reason}`);
			bodyToProcess = lastKnownStopEventBody; // Update bodyToProcess
            clearLastKnownStopEvent(); // Consumed the global one
		}

		outputChannel.appendLine(`[${operationName}] DAP Stop event received (Reason: ${bodyToProcess?.reason}), starting polling check...`);
		...
		outputChannel.appendLine(`[${operationName}] Calling processStopEvent with body (Reason: ${bodyToProcess?.reason})...`);
		const processedResult = await processStopEvent(currentActiveSessionAfterPoll, bodyToProcess, operationName);
		outputChannel.appendLine(`[${operationName}] Received processed result from processStopEvent: ${stringifySafe(processedResult)}`);
		
		const finalDapOutput = getActiveSessionDapOutput(currentActiveSessionAfterPoll.id) || getLastSessionDapOutput(currentActiveSessionAfterPoll.id);
		const finalRawOutput = rawTerminalOutputManager.getActiveSessionRawTerminalOutput(currentActiveSessionAfterPoll.id) || rawTerminalOutputManager.getFinalizedSessionRawTerminalOutput(currentActiveSessionAfterPoll.id);
		...
		outputChannel.appendLine(`[ERROR][${operationName}] Unhandled error in _executeDapRequestAndProcessOutcome: ${error.message} ${stringifySafe(error)}.`);
		const dapOutput = getLastSessionDapOutput(activeSession.id);
		const rawOutput = rawTerminalOutputManager.getFinalizedSessionRawTerminalOutput(activeSession.id);
		return _buildNavigationResult({
			success: false,
...
async function _executeNavigationDapCommand(
	...
	if (!sessionFromArg || !activeSession || sessionFromArg.id !== activeSession.id) {
		...
		return _buildNavigationResult({ success: false, errorMessage: `Error ${operationName}: No active debug session.` }, operationName);
	}
	...
	try {
        ...
        const timeoutForStop = (dapCommand === 'continue' || dapCommand === 'goto') ? CONTINUE_TIMEOUT_MS : STEP_TIMEOUT_MS;
		...
		const rawOutput = rawTerminalOutputManager.getActiveSessionRawTerminalOutput(activeSession.id) || rawTerminalOutputManager.getFinalizedSessionRawTerminalOutput(activeSession.id);
		return _buildNavigationResult({
			success: false,
...
export async function jumpToLine(session: vscode.DebugSession | undefined, params: JumpParams): Promise<NavigationResult> {
	...
	try {
		...
		outputChannel.appendLine(`[${operationName}] Attempting to get 'gotoTargets' for ${frame.sourcePath}:${params.line}`);
		
		const targetsResponse = await new Promise<any>((resolveCustom, rejectCustom) => {
			const timeoutId = setTimeout(() => rejectCustom(new Error("DAP 'gotoTargets' request timed out")), STEP_TIMEOUT_MS);
			activeSession.customRequest('gotoTargets', gotoTargetsArgs)
				.then(
					(response) => { clearTimeout(timeoutId); resolveCustom(response); },
					(err: any) => { clearTimeout(timeoutId); rejectCustom(err); }
				);
		...
		if (!targetsResponse?.targets?.length) {
			return _buildNavigationResult({ success: false, errorMessage: `Could not find a valid jump target at ${frame.sourcePath}:${params.line}. Debugger might not support jumping to this location.` }, operationName);
		}
		const targetId = targetsResponse.targets[0].id;
		if (targetId === undefined) {
			return _buildNavigationResult({ success: false, errorMessage: 'Found jump target but it lacked a required ID.' }, operationName);
		}
		...
		if (navOutcome.success && navOutcome.frame) {
			outputChannel.appendLine(`[${operationName}] Jump processed. Stopped at: ${navOutcome.frame.sourcePath}:${navOutcome.frame.line} (Targeted ${params.line})`);
            if (navOutcome.frame.sourcePath === frame.sourcePath && Math.abs(navOutcome.frame.line - params.line) > 2) { 
                outputChannel.appendLine(`[WARN][${operationName}] Stopped line ${navOutcome.frame.line} differs significantly from target ${params.line}.`);
            }
		} else if (!navOutcome.success) {
            outputChannel.appendLine(`[ERROR][${operationName}] Jump command failed or session ended: ${navOutcome.errorMessage}`);
        }
		return navOutcome;
		...
		if (error.message?.includes('session has been terminated') || error.message?.includes('No active debug session')) {
