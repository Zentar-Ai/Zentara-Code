    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (isTimeout && attempt < maxRetries) {
                outputChannel.appendLine(`[WARN] DAP request '${request}' timed out after ${timeoutMs}ms, retrying (${attempt+1}/${maxRetries})...`);
...
export async function stackTrace(session: vscode.DebugSession): Promise<StackTraceResult> {
    try {
        ...
        const frames: StackFrameInfo[] = dapResponse.stackFrames.map((frame: any) => ({ // Using 'any' for DAP frame type flexibility
            id: frame.id,
...
export async function getStackFrameVariables(session: vscode.DebugSession, params: GetStackFrameVariablesParams): Promise<GetStackFrameVariablesResult> {
    try {
        ...
        if (!scopesResponse?.scopes) {
            outputChannel.appendLine('[WARN] getStackFrameVariables: Invalid or empty scopes received from DAP request.');
        ...
        for (const scope of scopesToFetch) {
            ...
            if (!variablesResponse?.variables) {
                outputChannel.appendLine(`[WARN] getStackFrameVariables: Invalid or empty variables received for scope '${scope.name}' (ref: ${scope.variablesReference}).`);
...
export async function getSource(session: vscode.DebugSession, params: GetSourceParams): Promise<GetSourceResult> {
    try {
        ...
        if (!target.source?.path || target.line === undefined) {
             outputChannel.appendLine(`[WARN] getSource: Invalid target received from gotoTargets request: ${JSON.stringify(target)}`);
