...
function runTests() {
	...
	var test1Passed = sortedArr1.every(function (val, index) {
		return val === [1, 2, 5, 8, 12][index]
	})
	if (test1Passed) {
		passedCount++
	} else {
	...
	var test3Passed = sortedArr3.every(function (val, index) {
		return val === [1][index]
	})
	if (test3Passed) {
		passedCount++
	...
	var test4Passed = sortedArr4.every(function (val, index) {
		return val === [3, 3, 3][index]
	})
	if (test4Passed) {
		passedCount++
	} else {
