...
export class RawTerminalOutputManager {
    ...
    constructor() {
        ...
        const generalCloseListener = vscode.window.onDidCloseTerminal(terminal => {
    ...
    public startCaptureForSession(sessionId: string, terminal: vscode.Terminal): void {
        ...
        const buffer: string[] = [];
        const dataListener = (vscode.window as any).onDidWriteTerminalData((e: { terminal: vscode.Terminal, data: string }) => {
        ...
        const closeListener = vscode.window.onDidCloseTerminal(closedTerminal => {
            if (closedTerminal === terminal) {
                if (this.activeSessionTerminalCaptures.has(sessionId)) {
                    outputChannel.appendLine(`RawTerminalOutputManager: Terminal for session ${sessionId} closed. Finalizing capture.`);
